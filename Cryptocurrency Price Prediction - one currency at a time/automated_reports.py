...
def load_data(symbol, NLHR04LATEYL0K16):
    url = 'https://www.alphavantage.co/query?function=CRYPTO_INTRADAY&symbol='+symbol+'&market=USD&interval=1min&outputsize=compact&apikey='+NLHR04LATEYL0K16
    r = requests.get(url)
    data = r.json()
...
with open('Crypto_Report_'+date_time+'.html', 'a') as f:
    for coin in COINS:
        df = load_data(coin, NLHR04LATEYL0K16)
